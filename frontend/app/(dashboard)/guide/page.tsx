...
import {
  ...
  Calculator, BookOpen, Download, TrendingUp, Cpu, Search
} from "lucide-react";
...
const steps = [
  ...
  {
    num: 3,
    title: "Add your purchase invoices",
    icon: ArrowDownCircle,
    color: "text-purple-400",
    bg: "bg-purple-950/40",
    desc: "Every purchase invoice from a supplier must be entered to claim Input Tax Credit (ITC). BeMyCa auto-classifies RCM and checks ITC expiry on every save.",
    actions: [
  ...
  {
    num: 4,
    title: "Review auto-classifications & ITC expiry alerts",
    icon: Cpu,
    color: "text-violet-400",
    bg: "bg-violet-950/40",
    desc: "BeMyCa flags RCM invoices and blocked ITC automatically. Review these before reconciliation to avoid claiming ineligible credit.",
    actions: [
    ...
    tip: "Section 16(4) time-bar: ITC cannot be claimed after the due date of the September return of the year following the financial year of the invoice.",
  },
  {
    num: 5,
    title: "Run GSTR-2B Reconciliation & Hard-Lock",
    icon: GitMerge,
    color: "text-yellow-400",
    bg: "bg-yellow-950/40",
    desc: "Compares your purchase entries against GSTR-2B, then stamps each invoice per Section 16(2)(aa) — the per-invoice 2B hard-lock.",
    actions: [
    ...
    statuses: [
      { label: "Matched", color: "bg-green-900 text-green-300", desc: "Confirmed in GSTR-2B. Safe to claim ITC." },
      { label: "Missing in 2B", color: "bg-red-900 text-red-300", desc: "Supplier hasn't filed. ITC excluded until they do." },
      { label: "Accepted with Risk", color: "bg-amber-900 text-amber-300", desc: "Included in ITC — you accepted the notice risk." },
      { label: "Unverified", color: "bg-slate-700 text-slate-300", desc: "Not yet reconciled. Excluded from GSTR-3B ITC." },
    ],
    tip: "Only claim ITC on Matched invoices. The hard-lock automatically handles exclusion — you don't need to manually adjust GSTR-3B figures.",
  },
  {
    num: 6,
    title: "Check mismatch report & compliance status",
    icon: Search,
    color: "text-emerald-400",
    bg: "bg-emerald-950/40",
    desc: "Before filing, verify that GSTR-1 and GSTR-3B outward tax figures don't diverge beyond the 1% GSTN ANSR threshold.",
    actions: [
    ...
    warning: "The filing sequencer blocks GSTR-3B if GSTR-1 is not yet filed for the same period. File GSTR-1 by the 11th first.",
  },
  {
    num: 7,
    ...
    color: "text-blue-400",
    bg: "bg-blue-950/40",
    desc: "GSTR-1 is your outward sales return — file it by the 11th. GSTR-3B filing is blocked until this is done.",
    actions: [
    ...
    warning: "GSTR-1 deadline: 11th of every month for monthly filers. GSTR-3B cannot be filed until GSTR-1 is filed.",
  },
  {
    num: 8,
    ...
    color: "text-green-400",
    bg: "bg-green-950/40",
    desc: "GSTR-3B is the summary return where you pay the net GST liability. Due by the 20th. ITC is automatically computed using your 2B hard-lock status.",
    actions: [
    ...
    warning: "GSTR-3B is blocked if GSTR-1 is unfiled or the mismatch threshold is exceeded. Resolve both before filing.",
  },
  {
    num: 9,
  ...
  },
  {
    ...
    bg: "bg-orange-950/40",
    desc: "GSTR-9 aggregates all 12 months of outward supplies, ITC, and tax paid. All monthly GSTR-3Bs must be filed before GSTR-9 can be computed.",
    actions: [
    ...
    warning: "GSTR-9 is blocked if any monthly GSTR-3B is unfiled. GSTR-9 is due by 31 December following the financial year end. Late fee: ₹200/day, capped at 0.25% of turnover.",
    tip: "Verify GSTR-9 figures against your GST portal data before submitting. Discrepancies between portal and BeMyCa mean some invoices weren't entered.",
  },
  {
    num: 12,
...
const faq = [
  ...
  },
  {
    ...
    a: "Under RCM, the buyer (not the seller) pays the GST directly to the government. BeMyCa auto-detects RCM based on HSN/SAC code and supplier name (Section 9(3) categories: GTA, legal services, arbitral tribunal, director fees, insurance agents, and others). You still need to pay the RCM tax directly and can claim it as ITC in the same period (if eligible).",
  },
  {
    q: "What does 'ITC lapsed' mean?",
    a: "Under Section 16(4), ITC on an invoice cannot be claimed after the due date of the GSTR-3B for September of the year following the financial year of that invoice. BeMyCa auto-blocks lapsed ITC and shows an alert 60 days before the deadline. Once lapsed, the credit is permanently lost — it cannot be carried forward.",
  },
  {
    q: "What is the GSTR-1 vs GSTR-3B mismatch check?",
    a: "GSTN's ANSR system compares outward tax in your GSTR-1 (invoice-level) against GSTR-3B (summary return). If the deviation exceeds 1%, GSTN may issue a notice. BeMyCa runs this check before allowing GSTR-3B filing and blocks it if the deviation is too high — so you can fix the discrepancy first.",
  },
  {
    q: "What is the GSTR-2B Hard-Lock?",
    a: "Per Section 16(2)(aa), ITC can only be claimed on invoices that appear in your GSTR-2B. The hard-lock stamps each purchase invoice as: Matched (confirmed in 2B, safe to claim), Missing-in-2B (excluded from ITC until supplier files), or Accepted-with-Risk (you've chosen to claim despite absence in 2B, accepting notice risk). GSTR-3B automatically uses only locked-eligible ITC.",
  },
  {
    q: "Why was my GSTR-3B filing blocked?",
    a: "GSTR-3B can be blocked for three reasons: (1) GSTR-1 for the same period is not yet filed — file it by the 11th first; (2) GSTR-1 vs GSTR-3B outward tax deviation exceeds 1% — fix the invoice discrepancy; (3) GSTR-2B hard-lock hasn't been applied — run reconciliation and apply the lock first. Check Returns → Compliance Status to see which step is failing.",
  },
];
        ...
        <CardContent>
          <div className="flex items-center gap-2 flex-wrap text-sm">
            {[
              ...
              { label: "→", color: "" },
              { label: "Auto RCM + ITC Alerts", color: "bg-violet-900 text-violet-300" },
              { label: "→", color: "" },
              { label: "2B Recon + Hard-Lock", color: "bg-purple-900 text-purple-300" },
              { label: "→", color: "" },
              { label: "Check Mismatch & Compliance", color: "bg-emerald-900 text-emerald-300" },
              { label: "→", color: "" },
